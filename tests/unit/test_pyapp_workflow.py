...
def test_pyapp_build_workflow_uses_embedded_wheel():
    ...
    assert "WHEEL_ABS_PATH=" in run_script, "Should compute absolute path to wheel file"

    # Verify Python distribution is embedded (no runtime download)
    assert (
        'PYAPP_DISTRIBUTION_EMBED="true"' in run_script
    ...
    # VARIANT_CPU is only meaningful on Linux, but we only assert that it is set correctly.
    assert (
