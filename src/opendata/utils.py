...
def get_resource_path(relative_path: str) -> Path:
    """Get absolute path to resource, works for dev, PyInstaller and installed mode (pyApp)"""
...
def get_app_version() -> str:
    ...
    # 3. Append git SHA only in development mode (not bundled, not installed, .git exists)
    # Check if we're in a git repository near the source code
    package_root = Path(__file__).parent.absolute()
    project_src = package_root.parent
    is_dev_mode = project_src.name == "src" and (project_src.parent / ".git").exists()

    if not getattr(sys, "frozen", False) and is_dev_mode:
        try:
            import subprocess
