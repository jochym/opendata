...
def create_client_secrets(root_dir):
    ...
    if not client_id or not client_secret:
        print("⚠️  Google credentials not found in environment - skipping client_secrets.json")
        return None
...
def build_binary(platform_name, output_name=None, test_binary=False):
    ...
    cmd.extend([
        "--add-data", f"{ui_path}{separator}opendata/ui",
        "--add-data", f"{prompts_path}{separator}opendata/prompts",
        "--add-data", f"{version_file}{separator}.",
    ])
    ...
    excludes = [
        ...
        "IPython", "PIL", "tkinter", "test", "unittest",
    ]
